...
def load_model_mae(args, ckpt_path):
    ...
    model = models_mae_afno_8_8_finetune_next.__dict__[args.model](norm_pix_loss=norm_pix_loss, img_size=args.img_size,patch_size=args.patch_size)
    model.to(device)
    model_without_ddp = model
    print("Model = %s" % str(model_without_ddp))
    # print(str(args.save_dir) + "/" + "inference.log")
    sys.stdout = Logger("./inference_mae_16_16_207.log")
...
def autoregressive_inference(params, ic, valid_data_full, model, ff):
    ic = int(ic)
        ...
        for i in range(valid_data.shape[0]):
            ...
            if params.log_to_screen:
                ...
                logging.info('Predicted timestep {} of {}. {} RMS Error: {}, ACC: {}'.format(i, prediction_length, fld,
                                                                                             valid_loss[i, idx],
                                                                                             acc[i, idx]))
                ff.write('Predicted timestep {} of {}. {} RMS Error: {}, ACC: {}'.format(i, prediction_length, fld,
                                                                                             valid_loss[i, idx],
                                                                                             acc[i, idx]))
                if params.interp > 0:
                    logging.info(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--run_num", default='afno_8_precip', type=str)
    parser.add_argument("--yaml_config", default='./config/AFNO_afno_8_8_finetune_next.yaml', type=str)
    ...
    parser.add_argument("--override_dir",
                        default='/home/manxin/glh/mae/output/f_afno_8_8_next_614/finetuning/ckpt/',
                        type=str,
                        help='Path to store inference outputs; must also set --weights arg')
    ...
    # Set up directory
    for iii in range(1000, 9000): #718 725 717 679
        # if iii%5 == 0:
        #     continue
        ff = open(r"/home/manxin/glh/mae/output/f_afno_8_8_next_614/finetuning/inference_for_valid.txt", "a", encoding="UTF-8")
        ...
        ff.write(str(iii)+'\n')
        flag_mx = 'true'
        args.weights = '/home/manxin/glh/mae/output/f_afno_8_8_next_614/finetuning/ckpt/checkpoint-' + str(iii) + '.pth'
